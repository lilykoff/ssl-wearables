...
# Add a small helper somewhere near other helpers (top-level)
def save_label_mapping_json(report_path, label_encoder, logger=None):
    """
    Save a mapping from class label (string) -> integer index used by sklearn LabelEncoder.
    Writes to <report_path> with suffix '_label_mapping.json'.
    """
    mapping_path = report_path.replace(".csv", "_label_mapping.json")

    # LabelEncoder uses classes_ in index order
...
def evaluate_mlp(X_feats, y, cfg, my_device, logger, groups=None):
    ...
    if cfg.data.task_type == "classify":
        ...
        le.fit(y)
        y = le.transform(y)
        save_label_mapping_json(cfg.report_path, le, logger=logger)
    else:
        y = y * 1.0
